...
深度缓冲是在片段着色器运行之后（以及模板测试(Stencil Testing)运行之后，我们将在[下一节](https://learnopengl-cn.github.io/04%20Advanced%20OpenGL/02 Stencil testing/)中讨论）在屏幕空间中运行的。屏幕空间坐标与通过OpenGL的glViewport所定义的视口密切相关，并且可以直接使用GLSL内建变量gl_FragCoord从片段着色器中直接访问。gl_FragCoord的x和y分量代表了片段的屏幕空间坐标（其中(0, 0)位于左下角）。gl_FragCoord中也包含了一个z分量，它包含了片段真正的深度值。z值就是需要与深度缓冲内容所对比的那个值。
...
![img](Depth_testing.assets/depth_testing_func_always.png)

将它重新设置为GL_LESS，这会将场景还原为原有的样子：

![img](Depth_testing.assets/depth_testing_func_less.png)
...
*F**d**e**p**t**h*=*z*−*n**e**a**r**f**a**r*−*n**e**a**r*
...
值是我们之前提供给投影矩阵设置可视平截头体的（见[坐标系统](https://learnopengl-cn.github.io/01 Getting started/08 Coordinate Systems/)）那个 *near* 和 *far* 值。这个方程需要平截头体中的一个z值，并将它变换到了[0, 1]的范围中。z值和对应的深度值之间的关系可以在下图中看到：

![img](Depth_testing.assets/depth_linear_graph.png)
...
*F**d**e**p**t**h*=1/*z*−1/*n**e**a**r*1/*f**a**r*−1/*n**e**a**r*
...
![img](Depth_testing.assets/depth_non_linear_graph.png)
...
![img](Depth_testing.assets/depth_testing_visible_depth.png)
...
![img](Depth_testing.assets/depth_testing_visible_linear.png)
...
![img](Depth_testing.assets/depth_testing_z_fighting.png)
