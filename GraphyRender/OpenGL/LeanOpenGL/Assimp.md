...
![img](Assimp.assets/assimp_structure.png)
...
在[下一节](https://learnopengl-cn.github.io/03%20Model%20Loading/02 Mesh/)中，我们将创建我们自己的Model和Mesh类来加载并使用刚刚介绍的结构储存导入后的模型。如果我们想要绘制一个模型，我们不需要将整个模型渲染为一个整体，只需要渲染组成模型的每个独立的网格就可以了。然而，在我们开始导入模型之前，我们首先需要将Assimp包含到我们的工程当中。
...
现在，你应该已经编译完Assimp库并将它链接到你的程序中了。下一步：[导入](https://learnopengl-cn.github.io/03%20Model%20Loading/02 Mesh/)漂亮的3D物体！